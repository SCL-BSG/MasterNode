...
{
    // Bank SocietyG: check prefix
    if(uri.scheme() != QString("SOCG"))
        return false;
...
{
    // Convert SocietyG:// to SocietyG:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("SocietyG://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "SocietyG:");
    }
...
void openSocietyGConfigfile()
{
    boost::filesystem::path pathDebug = GetDataDir() / "societyG.conf";

    /* Open societyG.conf with the associated application */
    if (boost::filesystem::exists(pathDebug))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathDebug)));
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SocietyG.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SocietyG.lnk
...
{
    return GetAutostartDir() / "SocietyG.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a SocietyG.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SocietyG\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the SocietyG app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add SocietyG app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SocietyG-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SocietyG-qt [" + tr("command-line options") + "]                     " + "\n";
