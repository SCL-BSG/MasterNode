...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSocietyGAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSocietyGAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSocietyGAmount = strAnonymizeSocietyGAmount.remove(strAnonymizeSocietyGAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSocietyGAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSocietyGAmount*COIN) nMaxToAnonymize = nAnonymizeSocietyGAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSocietyGAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(nAnonymizeSocietyGAmount));
        //strAnonymizeSocietyGAmount = strAnonymizeSocietyGAmount.remove(strAnonymizeSocietyGAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
