...
const QString BITCOIN_IPC_PREFIX("SocietyG:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        LogPrintf("*** RGP QT paymentserver Loop 1 \n");
    ...
    {
        LogPrintf("*** RGP QT paymentserver Loop 2 \n");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start SocietyG: click-to-pay handler");
    else
...
void PaymentServer::handleURIConnection()
{
    ...
    LogPrintf("*** RGP Paymentserver, handleURIConnection \n");
