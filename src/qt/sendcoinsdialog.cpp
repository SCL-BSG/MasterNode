    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a SocietyG address (e.g. X8gZqgY4r2RoEdqYk3QsAqFckyf9pRHN6i)"));
#endif
...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    LogPrintf("*** RGP *** WalletModel::SendCoinsReturn WalletModel::sendCoins being called from sendCoinsDialogue \n");
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CSocietyGcoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 1000 SocietyG.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CSocietyGcoinAddress addr = CSocietyGcoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid SocietyG address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
