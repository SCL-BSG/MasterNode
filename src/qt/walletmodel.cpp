...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CSocietyGcoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CSocietyGcoinAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    LogPrintf("*** RGP *** \n");
    LogPrintf("*** RGP *** WalletModel::SendCoinsReturn WalletModel::sendCoins \n");


    if(isAnonymizeOnlyUnlocked())
    {

        LogPrintf("*** RGP *** SendCoinsReturn WalletModel::sendCoins iS isAnonymizeOnlyUnlocked \n");
        return AnonymizeOnlyUnlocked;
    }
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CSocietyGcoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CSocietyGcoinAddress(strAddress).Get();
    ...
    checkBalanceChanged(); // update balance immediately, otherwise there could be a short noticeable delay until pollBalanceChanged hits
     ...
     LogPrintf("*** RGP *** WalletModel::SendCoinsReturn WalletModel::sendCoins SUCCESS \n");
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CSocietyGcoinAddress(address).ToString());
...
{
    // SocietyG context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CSocietyGcoinAddress(address).ToString())].push_back(out);
    }
}
