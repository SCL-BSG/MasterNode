...
void CWalletDB::ListAccountCreditDebit(const string& strAccount, list<CAccountingEntry>& entries)
{
    ...
    while (true)
    {
        ...
        LogPrintf("*** RGP ListAccountCreditDebit while loop \n");
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CSocietyGcoinAddress(strAddress).Get()];
        }
        else if (strType == "tx")
        {
            uint256 hash;
    ...
    while (!fRequestShutdown)
    {
        ...
        if (nLastFlushed != nWalletDBUpdated && GetTime() - nLastWalletUpdate >= 2)
        {
            ...
            if (lockDb)
            {
                ...
                {
                    LogPrintf("*** RGP ThreadFlushWalletDB while loop \n");
                    nRefCount += (*mi).second;
                    mi++;
                }
